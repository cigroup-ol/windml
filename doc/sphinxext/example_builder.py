...
class ExampleBuilder:
    ...
    def parse_directory(self, path):
        ...
        subdirs = [d for d in L if os.path.isdir(os.path.join(dirpath, d)) and not d.endswith('.svn')]
        scripts = [s for s in L if s.endswith('.py') and not s.endswith('__init__.py')]
