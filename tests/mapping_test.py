...
class TestMapping(unittest.TestCase):
    ...
    def test_power_get_features_mill(self):
        X = self.pmapping.get_features_mill(self.windmill, 3, 3)
        assert(X[0][2] == X[1][1])
        assert(X[0][1] == X[1][0])

    def test_power_get_labels_mill(self):
        Y = self.pmapping.get_labels_mill(self.windmill, 3, 3)
        assert(len(Y) == 105259)
    ...
    def test_power_diff_mapping_get_features_mill(self):
        X = self.pdmapping.get_features_mill(self.windmill, 3, 3)
        assert(X.shape == (105259, 5))

    def test_power_diff_mapping_get_labels_mill(self):
        Y = self.pdmapping.get_labels_mill(self.windmill, 3, 3)
        assert(len(Y) == 105259)
